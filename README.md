...
该部分内容描述了Pseudo Data的数据格式，以及Pseudo Data与FIGER数据集、OntoNotes数据集之间的映射关系。

## Pseudo Data ##

该数据格式如下。key-"tokens"代表该条数据的输入语句，以空格拼接可以得到原始的语句;key-"mentions"储存语句中的每个mention及其label，每条记录包含mention的开始及结束位置(左闭右开)，以及相应的label.
...
`data_mapping.py`承担不同数据集之间label的映射工作，调用方式如下:
...
**mapping_figer.csv** 以及 **mapping_onto.csv** 为实验中所使用的映射表，每行包含两个string,以制表符分隔。第一个string表示在目标数据上(FIGER或OntoNotes)的label，第二个string表示在Pseudo Data上的label。

 **注:若pseudo Data的某个label无法被映射到目标数据上，我们将会把它映射到一个合适的上级类别，以保证数据的有效性。**
